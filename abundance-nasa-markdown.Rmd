...
Clean up datasets so that they are streamlined and functional for figure generation
```{r}
...
Create a dataframe that combines SWD data with weather data for each farm. A right_join is used to maintain all weather data even for dates that do not have N_swd counts, so that T2M remains continuous for graphing.
```{r}
AbundanceTempFBF<-right_join(SWD_data, FBF_weather, by = c("Coordinates","YYYYMMDD"))
AbundanceTempABF<-right_join(SWD_data, ABF_weather, by = c("Coordinates","YYYYMMDD"))
AbundanceTempCM<-right_join(SWD_data, CM_weather, by = c("Coordinates","YYYYMMDD"))
...
Use charts to visualize the relationship between SWD abundance and NASA temperature data. Stacked line and dot plots visualize temperature (T2M) and SWD population (N_swd) over time. Cross-correlation plots show the strength of correlation between T2M and N_swd at different time points, with temperature constant and N_swd lagging.
...
Finally, we run an equation equation that calculates the correlation between N_swd and T2M at day zero, showcasing the strength of the relationship with no lag.
```{r}
...
a<- ccf(AbundanceTempFrSa$N_swd,AbundanceTempFrSa$T2M, ylim = c(-0.5, .5), lag.max = 60, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.pass,
