...
#TESTING Create a subset of the SWD data for just FBF
SWD_dataFBF<-SWD_data %>% 
...
swd_interpolated<-approx(y = N_swd, x = YYYYMMDD, method = "linear", n = 50,
          rule = 1, f = 0, ties = mean, na.rm = TRUE)
#create interpolated data for VT flies
swd_interpolatedVT<-approx(y = SWD_dataVT$N_swd, x = SWD_dataVT$YYYYMMDD, method = "linear", n = 50,
          rule = 1, f = 0, ties = mean, na.rm = TRUE)
#create interpolated data for VA flies
swd_interpolatedVA<-approx(y = SWD_dataVA$N_swd, x = SWD_dataVA$YYYYMMDD, method = "linear", n = 50,
          rule = 1, f = 0, ties = mean, na.rm = TRUE)
#create interpolated data for KY flies
swd_interpolatedKY<-approx(y = SWD_dataKY$N_swd, x = SWD_dataKY$YYYYMMDD, method = "linear", n = 50,
          rule = 1, f = 0, ties = mean, na.rm = TRUE)

#TESTING create interpolated data for FBF flies
swd_interpolatedFBF<-approx(y = SWD_dataFBF$N_swd, x = SWD_dataFBF$YYYYMMDD, method = "linear", n = 50,
          rule = 1, f = 0, ties = mean, na.rm = TRUE)
```

```{r}
#create a cross correlation for SWD pop over time for full dataset
ccf(SWD_data$YYYYMMDD,swd_interpolated[["y"]], lag.max = NULL, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
    main = "SWD Population & Time cross-correlation")

#create a cross correlation for SWD pop over time for VT data
ccf(SWD_dataVT$YYYYMMDD,swd_interpolatedVT[["y"]], lag.max = NULL, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
    main = "VT SWD Population & Time cross-correlation")

#create a cross correlation for SWD pop over time for VA data
ccf(SWD_dataVA$YYYYMMDD,swd_interpolatedVA[["y"]], lag.max = NULL, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
    main = "VA SWD Population & Time cross-correlation")

#create a cross correlation for SWD pop over time for KY data
ccf(SWD_dataKY$YYYYMMDD,swd_interpolatedKY[["y"]], lag.max = NULL, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
    main = "KY SWD Population & Time cross-correlation")

#TESTING create a cross correlation for SWD pop over time for Sweet Roots data
ccf(SWD_dataFBF$YYYYMMDD,swd_interpolatedFBF[["y"]], lag.max = NULL, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
