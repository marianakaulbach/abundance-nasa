...
N_swd_interpolated <- na.approx(AbundanceTempTotal$N_swd, AbundanceTempTotal$YYYYMMDD, na.rm = TRUE, rule=2)
date <- AbundanceTempTotal$YYYYMMDD
# this code finds dates that have N_swd values that are not NA
...
library(dplyr)
swd_interpolated_df <- swd_interpolated_df %>%
        mutate(N_swd = na.approx(N_swd))
