...
colnames(swd_interpolated2VT)[colnames(swd_interpolated2VT) == "N_swd_interpolatedVT"] <- "N_swd"
swd_interpolated2<-na.omit(swd_interpolated2)

#create interpolated data for VT flies, filling in gaps
N_swd_interpolatedVT <- na.approx(AbundanceTempVT$N_swd, na.rm = TRUE, rule=2,maxgap=60)
date <- AbundanceTempVT$YYYYMMDD

swd_interpolated2VT <-data.frame(N_swd_interpolatedVT = N_swd_interpolatedVT, YYYYMMDD = date)
colnames(swd_interpolated2VT)[colnames(swd_interpolated2VT) == "N_swd_interpolatedVT"] <- "N_swd"
swd_interpolated2VT<-na.omit(swd_interpolated2VT)
...
ccf(swd_interpolatedFrSa[["y"]],swd_interpolatedFrSa[["x"]], ylim = c(-0.5, .5), lag.max = 30, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
    main = "FrSa Population & Temperature cross-correlation")

#create a cross correlation for SWD pop over time for Albe
ccf(swd_interpolatedAlBe[["y"]],swd_interpolatedAlBe[["x"]],ylim = c(-0.5, .5), lag.max = 30, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
    main = "AlBe Population & Temperature cross-correlation")

#create a cross correlation for SWD pop over time for LBA
ccf(swd_interpolatedLBA[["y"]],swd_interpolatedLBA[["x"]],ylim = c(-0.5, .5), lag.max = 30, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
    main = "LBA SWD Population & Temperature cross-correlation")

#create a cross correlation for SWD pop over time for CM
ccf(swd_interpolatedCM[["y"]],swd_interpolatedCM[["x"]],ylim = c(-0.5, .5), lag.max = 30, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
    main = "CM SWD Population & Temperature cross-correlation")

#create a cross correlation for SWD pop over time for ABF data
ccf(swd_interpolatedABF[["y"]],swd_interpolatedABF[["x"]], ylim = c(-0.5, .5),lag.max = 60, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
...
ccf(swd_interpolatedFBF[["y"]],swd_interpolatedFBF[["x"]], ylim = c(-0.5, .5), lag.max = 30, type = c("correlation", "covariance"),
    plot = TRUE, na.action = na.fail,
