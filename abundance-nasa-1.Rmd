...
```{r}
#download weather data for VT sites
...
```{r}
#download weather data for KY Sites
...
```{r}
#download weather data for VA Sites
...
AllFarmsVA_weather <-rbind(AlBe_weather,KiBa_weather,JoNu_weather,RaPo_weather,FeAb_weather,WiHa_weather,NoSh_weather,LiBl_weather,GwMe_weather,FrSa_weather,AlBr_weather,NaMo_weather,NaCa_weather,SiKi_weather, DoSc_weather,JoNu_weather)
```

```{r}
#Combine dataframes for all farms 
AllFarmsTotal_weather <-rbind(AllFarmsKY_weather,AllFarmsVA_weather,AllFarmsVT_weather)
```
...
```{r}
AllFarmsVT_weather$YEAR<-NULL
AllFarmsVT_weather$DOY<-NULL
AllFarmsKY_weather$YEAR<-NULL
AllFarmsKY_weather$DOY<-NULL
AllFarmsVA_weather$YEAR<-NULL
AllFarmsVA_weather$DOY<-NULL

#save YYYYMMDD as a date 
AllFarmsVA_weather$YYYYMMDD <- as.Date(AllFarmsVT_weather$YYYYMMDD, format = "%Y-%m-%d")
...
```{r}
#Combine dataframes for SWD and weather data
...
AbundanceTempVA<-merge(SWD_data, AllFarmsKY_weather, by = c("YYYYMMDD","Coordinates"))
AbundanceTempTotal<-merge(SWD_data, AllFarmsTotal_weather, by = c("YYYYMMDD","Coordinates"))
```
...
```{r}
#create a scatter plot of SWD Population over time
attach(AbundanceTempTotal)
plot(YYYYMMDD, N_swd,
     ylab = "Number of SWD Males",
     xlab = "Month",
     main = "SWD Population over Time",
     pch = 21,
     bg = "olivedrab",
     col = "olivedrab",
)
